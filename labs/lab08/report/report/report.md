...
Решим задачу линейного программирования ([@fig:001-@fig:004]).
...
Решим задачу линейного программирования, используя векторизированную линейную алгебру ([@fig:005-@fig:007]).
...
Решим задачу оптимизации описанного выше рациона питания ([@fig:008-@fig:014]).
...
Найдем наименьшее число паспортов, необходимых для путешествия по любой стране мира ([@fig:015-@fig:021]).
...
Найдем оптимальное число инвестиций в ценные бумаги, при получении хотя бы 2% возврата ([@fig:022-@fig:031]).
...
Восстановим изображения, решая задачу оптимизации ([@fig:032-@fig:040]).
...
Найдем значения переменных $x_1, x_2, x_3$ в данной оптимизационной задаче ([@fig:041,@fig:042])
...
Решим предыдущее задание, используя массивы вместо скалярных переменных ([@fig:043,@fig:044]).
...
Найдем значения вектора $\overrightarrow{x}$ в данной оптимизационной задаче ([@fig:045-@fig:048])
...
Согласно заданию ([@fig:049]), будем считать, что приоритеты по секциям у участников, кроме отказа от секции, не могут повторяться ([@fig:050,@fig:051]). После чего зададим и решим оптимизационную задачу ([@fig:052-@fig:054]), найдя оптимальную рассадку по секциям ([@fig:055]).
...
Переформулируем задачу. Введем обозначения: $x$ --- число приготовленных "Раф кофе", $y$ --- число приготовленных "Каппучино". Тогда необходимо решить следующую оптимизационную задачу:
...
при заданных ограничениях
$$
40x + 30y \leq 500, \quad 140x + 120y \leq 2000, \quad 5x = 40, \quad x \geq 0, \quad y \geq 0,
$$
где первое ограничение обозначает массу затраченных зёрен, второе --- массу затраченного молока, третье --- ванильного сахара.

Теперь решим полученную задачу, найдя $x$ и $y$ ([@fig:056,@fig:057]).
